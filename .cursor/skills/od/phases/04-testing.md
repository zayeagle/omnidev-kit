...
Phase 4 **不是**「跑几个冒烟用例」。按 [test-strategy.md](../engine/test-strategy.md) 自动组合并 **强制执行**：

| Layer | 默认 | 说明 |
|-------|------|------|
| **UNIT** | ✅ 必做、阻塞 | Phase 3 应已编写；缺失则 Gap Backfill |
| **INT** | 多模块/API 时必做 | supertest / testcontainers / 仓库既有模式 |
| **SYS** | L/XL 多服务 | docker-compose + API suite |
| **E2E** | 全栈必做 | **Playwright** 优先；Browser MCP / E2E sub-agent 可选 |
| **SMK** | ✅ 必做 | 关键路径子集 |
| **REG** | ✅ 定向 | Module 标签；L/XL 发布可全量 |

**输出**: inline 结果写入 `05-test-plan.md` + `05-test-report.md` + metrics `test_complete`
...
**Interactive** (if layers disputed): confirm via [interactive-prompt.md](../engine/interactive-prompt.md) — e.g. skip E2E requires B.0 explicit opt-out when `e2e_required_fullstack: true`
...
2. **Interactive prompt**: 回补上游文档 / 当场补充 / 跳过(须理由)
3. Actions:
