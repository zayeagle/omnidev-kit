...
$checks = @(
  @{ Path = "skills\od\engine\trigger-gate.md"; Need = @('[\/$]od', 'Explicit non-activation feedback', 'STOP') },
  @{ Path = "skills\od\engine\activation.md"; Need = @('[\/$]od', 'STOP', 'AskUserQuestion', 'WAIT') },
  @{ Path = "skills\od\engine\interactive-prompt.md"; Need = @('STOP', 'WAIT', 'AskQuestion', 'phase0_s_fastpath', 'Decision Matrix', 'deploy_consent', 'Markdown Fallback Table', 'box-drawing', 'codex_auto_resolve', 'allow_auto_resolve') },
